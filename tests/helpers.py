...
def collect_function_ids(extracted_dir: Path):
    ...
    for func_dir in extracted_dir.iterdir():
        ...
        for f in func_dir.iterdir():
            # Skip .spec.json/.info.json metadata sidecars and other JSON files.
...
def write_dummy_sidecars(target: Path):
    """Write valid dummy .spec.json/.info.json sidecars for extracted functions.

    ``target`` may be a single extracted-function file or an
    ``extracted_functions`` directory (walked recursively). Existing metadata
    sidecars and other .json files are left alone. Returns the list of function
    files that received sidecars.
    """
    target = Path(target)
    if target.is_dir():
        files = [
    ...
    for func_file in files:
        spec = {
            "signature": f"{func_file.stem}(...) -> void",
            "pre_condition": "The function is called with valid arguments.",
            "post_condition": "The function satisfies its contract.",
        }
        info = {"callees": []}
        Path(str(func_file) + ".spec.json").write_text(
            json.dumps(spec, indent=2), encoding="utf-8"
        )
        Path(str(func_file) + ".info.json").write_text(
            json.dumps(info, indent=2), encoding="utf-8"
        )
        written.append(func_file)
    return written
