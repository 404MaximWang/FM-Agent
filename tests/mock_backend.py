...
from tests.helpers import DEFAULT_SCENARIO, load_scenario, write_dummy_sidecars
...
class FixtureBackend:
    ...
    def _materialize_function_outputs(
        ...
        if kind == "sidecars":
            # Specs live in .spec.json/.info.json metadata sidecars next to each
            # extracted function file; synthesize valid ones for every function
            # the stage was asked to spec.
            to_root = Path(spec.get("to_root", "extracted_functions"))
            for fid in function_ids:
                file_slug, func_name = fid.split("::")
                func_dir = work_dir / to_root / file_slug
                matches = [
                    path for path in func_dir.glob(f"{func_name}.*")
                    if path.suffix != ".json"
                ]
                if not matches:
                    raise KeyError(f"No workspace extracted file for {fid}")
                write_dummy_sidecars(matches[0])
            return
