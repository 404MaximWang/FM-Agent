...
def test_partial_spec_attempt_does_not_drain_unready_files(
    ...
    import main
    from src import spec_generation_and_verification as sgv
    monkeypatch.setattr(sgv, "OPENCODE_MAX_RETRIES", 2)
    monkeypatch.setattr(sgv.time, "sleep", lambda _seconds: None)
    ...
    def fake_streaming_reasoner(
        ...
        del already_processed, resume, backend, bug_validator_path
    ...
    monkeypatch.setattr(sgv, "streaming_reasoner", fake_streaming_reasoner)
...
def test_verify_pre_specced_functions(mock_backend, tmp_path):
    ...
    write_dummy_sidecars(input_dir)
